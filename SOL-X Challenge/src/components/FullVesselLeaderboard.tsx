...
import { Trophy, Medal, Award, ArrowLeft, Ship, Clock } from 'lucide-react';

interface FullVesselLeaderboardProps {
  challenge: Challenge;
  leaderboardEntry: LeaderboardEntry;
  onBack: () => void;
}

const FullVesselLeaderboard: React.FC<FullVesselLeaderboardProps> = ({ 
  challenge, 
  leaderboardEntry, 
  onBack 
}) => {
  ...
  const vesselData = leaderboardEntry.vesselLeaderboard;
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-6">
          <div className="flex items-center space-x-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-gray-400">Top {vesselData.length} Vessels</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 shadow-xl shadow-black/20 overflow-hidden">
          <div className={`bg-gradient-to-r ${challenge.gradient} p-6`}>
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-bold text-white">Vessel Leaderboard</h2>
            </div>
            <p className="text-white/80 mt-2">{getVesselMetricLabel(challenge.id)}</p>
          </div>
                ...
                {vesselData.map((vessel, index) => (
                  ...
                  >
                    <div className="flex items-center space-x-4 min-w-0 flex-1">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${getRankBadgeColor(vessel.rank)}`}>
                      ...
                      <div className="flex items-center space-x-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-white text-base truncate">{vessel.vessel_name}</p>
                          <p className="text-sm text-gray-400">Vessel</p>
                        </div>
                      </div>
                    </div>
...
export default FullVesselLeaderboard;